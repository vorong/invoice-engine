...
function runBatchConversion() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (currentTime - startTime > maxRunTimeMs) {
      console.log(
        `Time limit reached (${(currentTime - startTime) / 1000}s). Stopping batch.`,
      );
      break;
    }
...
function processSingleFile(
  ...
  try {
    if (
      !gDocId &&
      (ENABLE_CONVERT_GDOC || ENABLE_CONVERT_HTML || ENABLE_CONVERT_PDF)
    ) {
      const resource = {
        name: fileName,
    ...
    if (ENABLE_CONVERT_HTML && !htmlId) {
      ...
      htmlId = DriveApp.getFolderById(FOLDER_ID_INVOICE_CONVERTED_HTML)
        .createFile(response.getBlob())
        .setName(fileName + ".html")
        .getId();
    ...
    if (ENABLE_CONVERT_PDF && !pdfId) {
      ...
      pdfId = DriveApp.getFolderById(FOLDER_ID_INVOICE_CONVERTED_PDF)
        .createFile(response.getBlob())
  ...
  const resultsUpdate = [
    [finalStatus, gDocId || "", htmlId || "", pdfId || ""],
  ];
  sheet
    .getRange(row, COLUMN_NUMBER_FILE_REGISTRY_CONVERSION_STATUS, 1, 4)
    .setValues(resultsUpdate);
}
