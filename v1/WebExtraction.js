...
function runRawWebsiteScrape() {
  ...
  URLS_COMPANY_WEBSITE_TARGETS.forEach(url => {
    ...
    const safeText = cleanText.length > 49000 ? cleanText.substring(0, 49000) + "... [TRUNCATED]" : cleanText;
...
function runWebsiteKnowledgeExtraction() {
  console.log("Standing by for RAW data review before finalizing structured extraction.");
}